...
(async () => {
  try {
    ...
    Logger.error('UPLOAD', 'Failed to create uploads directory', error.message);
  }
})();
...
async function cleanupUploadedFiles(filePaths) {
  if (!filePaths) {
    try {
      ...
      Logger.warn('UPLOAD', "Error during cleanup", error.message);
    }
    return;
  }
...
function performGC() {
  try {
    ...
    Logger.warn('GC', 'Failed to perform garbage collection', error.message);
  }
}
